...
<details>
  <summary>Технические детали - click to open:</summary>
После запуска ВМ необходимо создать виртуальное окружение и установить необходмые библиотеки:
  ```
    sudo apt update
    python3 –version
    sudo apt-get install python3-pip
...
- Разобраться с ошибкой при запуске job на сервере datasphere из ВМ, так как при запуске из докальной CLI такой ошибки нет.
